...
// src/app/dashboard/page.tsx (Updated with Weekly Analytics)
"use client";
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const [
        ...
        apiClient.get("/experience"),
        apiClient.get("/certifications"),
        apiClient.get("/analytics/stats/weekly"),
      ]);
      ...
      setWeeklyStats(weeklyAnalytics);
    } catch (err: any) {
      console.error("Error fetching dashboard data:", err);
    } finally {
      setLoading(false);
      ...
      <div className="space-y-8">
        <section>
          ...
          <div className="grid md:grid-cols-4 gap-6">
            ...
            <StatCard
              title="Weekly Views"
              value={weeklyStats?.totalViews.toLocaleString() || "0"}
              icon="👁️"
              change={weeklyStats?.percentageChange}
            />
          </div>
        </section>
        ...
        {weeklyStats && (
          <section>
            <h3 className="text-2xl font-bold mb-4 text-gold">
              Weekly Analytics
              <span className="text-sm text-silver font-normal ml-3">
                {formatDate(weeklyStats.period.start)} - {formatDate(weeklyStats.period.end)}
              </span>
            </h3>
            ...
            <div className="grid md:grid-cols-3 gap-6 mb-6">
              <div className="bg-black border-2 border-gold p-6 rounded">
                <div className="flex justify-between items-start">
                  <div>
                    <p className="text-silver text-sm mb-1">Total Page Views</p>
                ...
                {weeklyStats.percentageChange !== 0 && (
                  <div className={`mt-2 text-sm ${
                    weeklyStats.percentageChange > 0 ? "text-green-500" : "text-red-500"
                  }`}>
                    {weeklyStats.percentageChange > 0 ? "↑" : "↓"}{" "}
                    {Math.abs(weeklyStats.percentageChange)}% vs last week
                  </div>
                )}
              </div>
              ...
              <div className="bg-black border-2 border-gold p-6 rounded">
                <div className="flex justify-between items-start">
                  <div>
                    <p className="text-silver text-sm mb-1">Unique Pages</p>
                    <p className="text-3xl font-bold text-white">
                      {weeklyStats.uniquePages}
                    </p>
                  </div>
                  <span className="text-3xl">📄</span>
                </div>
              ...
              <div className="bg-black border-2 border-gold p-6 rounded">
                <div className="flex justify-between items-start">
                  <div>
                    <p className="text-silver text-sm mb-1">Avg. Daily Views</p>
                    <p className="text-3xl font-bold text-white">
                      {Math.round(weeklyStats.totalViews / 7)}
                    </p>
                  </div>
                  <span className="text-3xl">📈</span>
                </div>
            ...
            {/* Daily Trend Chart */}
            <div className="bg-black border-2 border-gold p-6 rounded mb-6">
              <h4 className="text-xl font-semibold text-white mb-4">Daily Traffic Trend</h4>
              <div className="space-y-3">
                {weeklyStats.dailyStats.map((day, index) => {
                  const maxViews = Math.max(...weeklyStats.dailyStats.map(d => d.views));
                  const percentage = (day.views / maxViews) * 100;
                  
                  return (
                    <div key={index} className="space-y-1">
                      <div className="flex justify-between text-sm">
                        <span className="text-silver">{formatDate(day.date)}</span>
                        <span className="text-white font-semibold">{day.views} views</span>
                      </div>
            ...
            {/* Top Pages This Week */}
            <div className="bg-black border-2 border-gold p-6 rounded">
              <h4 className="text-xl font-semibold text-white mb-4">Top Pages This Week</h4>
              <div className="space-y-3">
                {weeklyStats.topPages.slice(0, 5).map((page, index) => (
                  <div
                    key={index}
                    className="flex justify-between items-center p-3 border border-gold rounded hover:bg-gray-900 transition-colors"
                  >
                    <div className="flex items-center gap-3">
                      <span className="text-gold font-bold text-lg">#{index + 1}</span>
                      <span className="text-silver font-mono text-sm">{page._id}</span>
                    </div>
        ...
        <section>
          <h3 className="text-2xl font-bold mb-4 text-gold">Quick Actions</h3>
          <div className="grid md:grid-cols-3 gap-4">
            <QuickActionButton icon="📝" label="Write New Article" href="/articles" />
            <QuickActionButton icon="🚀" label="Add Project" href="/projects" />
            <QuickActionButton
              icon="💬"
              label="Check Messages"
              href="/messages"
  ...
  const content = (
    ...
    >
      ...
      {change !== undefined && change !== 0 && (
        <p className={`text-sm mt-1 ${change > 0 ? "text-green-700" : "text-red-700"}`}>
...
function ContentBar({ label, count, max }: { label: string; count: number; max: number }) {
  const percentage = Math.min((count / max) * 100, 100);
...
function QuickActionButton({
  ...
  return (
    ...
    >
      <span className="text-2xl">{icon}</span>
      <span className="font-semibold text-gold group-hover:text-black">{label}</span>
      {badge && badge > 0 && (
        <span className="absolute top-2 right-2 bg-red-600 text-white text-xs font-bold px-2 py-1 rounded-full">
          {badge}
        </span>
